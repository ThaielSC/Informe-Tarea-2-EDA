...
Para Radix Sort, se implementó una función de mapeo (getPos) que convierte cada carácter alfanumérico en un entero para su correcta distribución. Esto se logró restando el carácter base (`'0'` para dígitos y `'A'` para letras mayúsculas), aprovechando la estructura de tamaño fijo del código postal (6 caracteres) para simplificar las llamadas encadenadas al algoritmo de conteo (`countingSort`).
...
En conclusión, Radix Sort demostró ser el algoritmo más eficiente en todos los escenarios de prueba. La ventaja de su complejidad cercana a $O(n)$ sobre el crecimiento $O(n log(n))$ de Quick Sort y Merge Sort garantiza que su superioridad en rendimiento sea aún más pronunciada con el incremento del tamaño de los datos.
